...
| D4 | Item-data = PDC under **many keys**, JSON enchant list, cache "bolted on," keyed by **ItemMeta identity** | **One compact stable-string-keyed record**, cached by **content-hash + generation counter** *(cache key as originally specified; shipped keyed by the raw blob string, deliberately not a hash — see §5.2)*; lore rendered from state | Meta-identity keys miss constantly and can alias (latent correctness bug per `[crit:perf]` §6). Content-hash + generation is the only cache-soundness story that survives ItemStack copies (`[do]`). |
...
| D7 | `tester` is a peer module, otherwise the gate is implicit | The gate is **two-layer and explicit** with a **`validateContent` CI task** *(as originally specified; shipped with no separate task — content is validated inside `./gradlew build`, see §10)* compiling the whole library against a fake resolver, plus golden-item/golden-config fixtures | Auditing hundreds of enchants becomes a reviewed, CI-gated diff, not a live-server gamble (`[ax]`). |
...
`WornStateStore` (§5.5) is the thirteenth per-player store but lives with its data in `se/item/src/item/worn/`
