...
- **Shipped:** `schema.diag.DiagCode` (producers + tests reference the enum; `E_PARSE`
  ...
  `Defs` (the ability/lowered builders), the runtime-record builders
  `Abilities`/`WornStates`/`Snapshots`, the sink-wiring `Envs`, the shared
  `SyncSchedulerBackend`/`RecordingSchedulerBackend` doubles, and the
  `PermissiveResolvers`/`Specs` structural-validation fixtures; the **46 effect-kind tests collapsed
...
| **`Abilities` / `WornStates` / `Snapshots`** (runtime record builders) | The hand-rolled 19-arg `Ability`, 8-arg `WornState`, and `Snapshot` constructions re-typed across the executor/pipeline/driver/worn tests — worse since ADR-0039 grew those records with `kindId`/`factMask` | The one place the runtime record arity (incl. the ADR-0039 `FactMask.ALL` / head-fallback defaults) lives: `Abilities.ability().id(3).trigger(ATTACK).effects(…).build()`, `WornStates.worn().gen(g).byTrigger(t, ids).build()`, `Snapshots.snapshot().abilities(…).stableKeys("enchants/x").build()`. Override-what-you-test; a real `Snapshot` replaces the per-test mock `Answer`. |
| **`Envs`** (SinkEnv/EngineStores wiring) | The telescoping `DispatchSink`/dispatcher ctors that each defaulted a **private shadow store** a separate-event reader never saw | `Envs.sink().vars(store).nowTicks(() -> 100L).build()` → a `SinkEnv` over an `EngineStores` aggregate with per-store override slots; unset slots get fresh instances. `.stores(aggregate)` overrides the whole aggregate. |
| **`SyncSchedulerBackend` + `RecordingSchedulerBackend`** | The four hand-rolled `SchedulerBackend` doubles copy-pasted across three modules | `SyncSchedulerBackend` runs every hop (delays + repeating) inline; `RecordingSchedulerBackend` captures `*Later` hops for `runDelayed()` and now **also** arms `repeating*` as cancellable `Repeat` handles (`backend.repeating.get(0).periodTicks`, `.isCancelled()`). |
| **`PermissiveResolvers`** | The five copy-pasted permissive `PlatformResolvers` anonymous classes at the structural-validation + migrator seams | `testfx.PermissiveResolvers.INSTANCE` — every handle token resolves to id 0 (structural validation only; existence is `CatalogSuite`'s job). `SignaturePackValidationTest`'s STRICT resolvers stay put. |
| **`Specs`** | The duplicated `SpecRegistry` → nullable `head→ParamSpec` lookup lambda the migrator writers take | `testfx.Specs.lookup(registry)`. The one production use keeps its inline lambda (prod cannot depend on `testfx`). |
