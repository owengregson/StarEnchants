...
a packet/anticheat reference plugin. StarEnchants is a different kind
of plugin (content-/engine-/GUI-driven). The user is explicit: the architecture
must be uniquely best for THIS plugin, not borrowed from a packet/anticheat
reference plugin or a Cosmic Enchants-style plugin.
...
mirror prior. The analysis of a Cosmic Enchants-style reference (local-only,
gitignored) informs WHAT to build and HOW features interact — never how to
construct it.
...
- Adopt the packet/anticheat-reference mirror layout directly — rejected: risks
  building on a borrowed, possibly inferior architecture.
