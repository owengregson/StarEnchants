...
2. **Players ride Paper's freeze-tick lock; mobs never do.** On Paper ≥1.18.2 (and all Folia, floor 1.19.4) a
   single `lockFreezeTicks(true)` + one `setFreezeTicks(max)` pins forever — fire included — because the lock
   guards BOTH vanilla writes (the decay and the burning-entity clear), so there is zero per-tick work and the
   behavior is identical on Paper and Folia. Mobs take an unlocked per-tick re-pin at `max + 2` (the `+2`
   outruns the −2/tick decay so the synced value is exactly max): `freezeLocked` persists to NBT, so a locked
   mob unloaded mid-window would strand frozen forever, while an unlocked pin self-heals by decay in ~70t. The
   1.17.1 floor has no lock, so a player there also degrades to the unlocked pin. The pin is **skipped while
   the target burns** on the unlocked path (the unguarded `baseTick` clear would zero it and replay the 1009
   hiss every tick) — the visual honestly drops until the fire ends. 1.8.9 has no freeze concept: the visual is
   a recorded no-op; DoT + slow still land.
...
3. **Vanilla freeze self-damage is cancelled for the window.** A full pin makes vanilla deal 1 damage every
   ...
   DoT is the ONLY damage source and it is uniform across victims. The same listener join-reconciles a
   crash-stranded `freezeLocked` (unlock + thaw a locked player with no live window). 1.8.9's binding is inert.
...
5. **Vanilla's frost slow is neutralized so the authored percent is the real slow.** At a full pin vanilla
   ...
   chain (the 1.21.3 rename). On 1.8.9 there is no vanilla frost, so the offset is never applied and the slow
   rides the NMS `GenericAttributes.MOVEMENT_SPEED` modifier path.
...
6. **Lifecycle.** Teardown (cancel tasks, end the visual, strip the slow, disarm the registry) is one
   ...
   the `SwarmSpawns.removeAll` best-effort shape). Natural expiry / death claim the token via the DoT task's
   liveness check. **Suppression:** Silence gates NEW procs at gate 5; a live window is sink state (not an
...
- The 1.17.1 floor and any mob honestly drop the visual while the victim burns (DoT + slow persist); a leather
