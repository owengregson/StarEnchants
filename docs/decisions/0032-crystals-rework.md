...
- `{CRYSTAL}` — the component crystal display name(s), styled, joined by the name template's leading colour + `, `
  (a single is just its own display name). A merged Chaos+Light renders `&4&lChaos&6&l, &e&lLight`.
