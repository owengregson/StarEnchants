...
The same `%name%` placeholders expand inside a `MESSAGE` effect and as the value of a `SET_VAR`. So you can write `{ MESSAGE: { text: "&7You hit %victim.type% for %damage%" } }` or stash a fact for a later condition.
:::
