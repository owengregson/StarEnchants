...
**Success dust** is dragged onto an enchant book to **raise its success chance** by a bonus, clamped to
the `books.max-success` ceiling in [config.yml](../configuring/index.mdx) (default `100`). It is pure
economy metadata — it never compiles to an ability.
