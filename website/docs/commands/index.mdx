...
`cosmic-pack` carries the whole surface — `content/` in full, every item likeness, and its own menu chrome —
but not all of it is a port. Thirteen item likenesses are recovered from the source down to their exact display
strings; the other nine (the soul gem, the four trak gems, the item nametag, the holy white scroll, the godly
transmog and the weapon reforge) have no counterpart to port, so they are styled to the pack instead. The menus
are theme rather than likeness for the same reason. Each file says which it is in its own header. See
[Configuring](../configuring/index.mdx) for the full note.
