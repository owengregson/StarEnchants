...
`cosmic-pack` does not carry every item yet: it ships `content/` in full plus four item likenesses, so 18
item likenesses and all menus stay at the engine defaults after you apply it. Nothing breaks — those items
just still look like stock StarEnchants. The rest are being decomposed from source and land as their own
batch. See [Configuring](../configuring/index.mdx) for the full note.
