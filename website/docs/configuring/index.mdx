...
shipped examples all live in the signature pack. See [Reforges](../items/reforges.mdx).
...
current one first. See the [commands](../commands/index.mdx) page.
...
**`cosmic-pack` carries the whole surface, but not all of it is a port.** Thirteen item likenesses
are recovered from the original down to their display strings — the crystal, the Heroic upgrade,
masks, pets, enchant books, mystery books, dust, the enchantment orb, and the
white/black/transmog/randomization scrolls and pet food. The other nine are port-original: the soul
gem, the four trak gems, the item nametag, the holy white scroll and the godly transmog were minted
by other plugins on the original server, so there is nothing of theirs to port, and weapon reforges
are ours alone. They still ship, styled to the pack, so the shelf reads as one set. The menus are
theme rather than likeness for the same reason — nothing about the original's GUIs is recorded.
Every file states which of the two it is in its own header.
