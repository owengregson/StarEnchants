...
**`cosmic-pack` does not carry every item yet.** It ships `content/` in full plus four item
likenesses (crystal, heroic, mask, pet). Applying it therefore leaves 18 item likenesses and all
menus at the engine defaults — nothing breaks, but those items still look like stock StarEnchants
rather than the ported originals. Those identities are being decomposed from source and land as
their own batch; none of them is guessed at.
