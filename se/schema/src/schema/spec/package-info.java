/**
 * The StarEnchants type system: one {@link schema.spec.ParamSpec}
 * per DSL kind, used four ways.
 *
 * <p>Each effect/condition/selector/trigger declares a {@code ParamSpec} —
 * an ordered list of typed {@link schema.spec.Param}s built from
 * the {@link schema.spec.D} vocabulary
 * ({@code D.DOUBLE.min(0).max(100)}). That single declaration drives validation
 * (→ typed {@link schema.spec.Args}), tab-completion, {@code /se
