 ...
 * ({@code { min: 2, max: 5 }}, {@code { nearly-maxed: 4 }}, {@code { ability-set: 4 }},
 * {@code { chance: 25, min: 1, max: 4 }}).
 *
 ...
 *               of a {@link Mode#NEARLY_MAXED} or {@link Mode#ABILITY_SET} draw
...
public record EnchantRoll(int min, int max, double chance, Mode mode) {
    ...
    public enum Mode {
        ...
        NEARLY_MAXED,
        /**
         * The ABILITY-SET draw (codex §A.11, R-QC64), the second of the family's two helpers: {@code M -
         * rand(min(4, M))} — a rung wider at the bottom than {@link #NEARLY_MAXED} — followed by a 25 % chance
         * to shave one level off a draw that landed on a natural max above 1. The four M-Kit sets mint through
         * it; every other set draws {@link #NEARLY_MAXED}.
         */
        ABILITY_SET
    }
