 ...
 * teleblock, a potion lock, a disarm — are plugin state with no cleanse at all, so a counterplay item aimed at
 * one had nothing to author. {@code TRAP_BREAK} is the sole precedent and it is scoped to physical confinement.
 *
...
public final class StatusClearEffect implements EffectKind {
    ...
    static final EffectSpec SPEC = EffectSpec.of("STATUS_CLEAR")
            .param("status", D.enumOf("TELEBLOCK", "POTION_LOCK", "DISARM"),
            ...
            .doc("Remove an active engine status window from each target: TELEBLOCK (the teleport denial), "
                    + "POTION_LOCK (every potion denial held on them), or DISARM (the armed disarm window). "
