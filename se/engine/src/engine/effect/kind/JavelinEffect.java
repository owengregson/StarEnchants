 ...
 * caster's facing at speed blocks/tick (AUTHORED slow — dodgeable by design), max-travel blocks
 * total. First living thing on the path: one weapon-swing's damage, knockback × the javelin's own
 * flight direction (straight back along its angle), a lock-ticks camera+movement hold (per-tick
 * position+look pin on the victim's own scheduler — the known-viable cross-version mechanism),
 * then Nausea. A wall stops it; a miss is wasted. Service-owned (the DIG_HOME rule).
 */
public final class JavelinEffect implements EffectKind {
    ...
    static final EffectSpec SPEC = EffectSpec.of(HEAD)
            ...
            .param("lock", D.TICKS.def(20), "camera-lock + movement-freeze length")
            .param("lock-delay", D.TICKS.def(5), "ticks after impact before the pin arms (lets the knock land)")
            ...
            .doc("Javelin (reforges): a straight particle javelin at speed blocks/tick, max-travel blocks. On "
                    + "the first living hit: one weapon-swing's damage (or FLAT damage), knockback × along the "
                    + "flight angle, a lock-tick camera+movement hold, then nausea. Deliberately slow — "
                    + "sidestepping it is the counterplay; a miss is wasted. Reforge-service-owned: this "
                    + "effect emits no intent of its own.")
