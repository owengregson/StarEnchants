...
public interface Sink {
    ...
    /**
     * Tell {@code recipient} a proc was eaten by an authored chance rebate (ADR-0076 part E), at most once per
     * player per {@code REBATE_THROTTLE_TICKS}. Throttled here for the same reason
     * {@link #outOfSoulsNotice} is: the rebated band widens with the DEFENCE, so a well-countered defender
     * trips several sibling abilities' bands on the same swing and would otherwise read one identical line per
     * ability per hit. Line and cue share the one throttle. {@code message} may be empty, {@code soundId} -1.
     */
    void rebateNotice(Player recipient, String message, int soundId);
     ...
     * a refund can only ever release the window this very ability armed. {@code targetBucket} and
     * {@code victim} are the OTHER halves of the key gate 6 wrote — the mob/player route and, for a
     * {@code cooldown-per-victim:} ability, its separate per-victim dimension — and reconstructing the key
     * without them releases nothing. The release is value-matched on the expiry the gate wrote, so a stale
     * call misses and leaves the window standing.
     *
     ...
     */
    void refundCooldown(Player actor, int scopeId, int targetBucket, UUID victim, int cooldownTicks);
}
