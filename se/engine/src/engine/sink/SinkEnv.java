...
public record SinkEnv(EconomyService economy, SoulDebit souls, EngineStores stores, LongSupplier nowTicks,
    ...
    /**
     * The {@code itemXp} shape: the {@code ITEM_XP_TRACK} seam the pets family implements
    ...
    public static SinkEnv of(EconomyService economy, SoulDebit souls, EngineStores stores, LongSupplier nowTicks,
                             ...
                             SummonPayloads payloads, SiteGate siteGate, ItemXpGrant itemXp) {
        return of(economy, souls, stores, nowTicks, movementExemption, moneyInterestCap, gearProtection,
                lightningBoost, visibility, permanentPotions, payloads, siteGate, itemXp, BlockVisibility.NONE);
    }

    /**
     * The full shape: {@code blockVisibility} is the {@code PHANTOM_BLOCKS} seam — per-viewer client-only block
     * overlays, wired from the era bindings because 1.8.9 has no {@code BlockData}
     * ({@link BlockVisibility#NONE} = no client ever sees a phantom).
     */
    public static SinkEnv of(EconomyService economy, SoulDebit souls, EngineStores stores, LongSupplier nowTicks,
                             Consumer<Player> movementExemption, DoubleSupplier moneyInterestCap,
                             GearProtection gearProtection, ToDoubleFunction<UUID> lightningBoost,
