 ...
 * here). Flags needing a live listener (target suppression, hit-gated detonation) register the spawn in
 * {@link PetSummons}; the feature layer enforces them.
 *
...
public record SummonFlags(boolean powered, boolean noAi, boolean noTarget, boolean saddled,
                          boolean mountActivator, boolean detonateOnPlayerHit) {

    public static final SummonFlags NONE = new SummonFlags(false, false, false, false, false, false);

    /** Whether any flag needs the {@link PetSummons} registry + the summon-guard listener. */
    public boolean tracked() {
        return noTarget || detonateOnPlayerHit;
    }
