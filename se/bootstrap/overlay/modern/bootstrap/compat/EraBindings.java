...
import feature.combat.FreezeJumpGuardListener;
...
public final class EraBindings implements EraServices {
    ...
    /** FREEZE no-jump guard (R-QC57): cancel a ground-pinned victim's jump; Paper's own cancellable event. */
    @Override
    public Listener freezeJumpGuard() {
        return new FreezeJumpGuardListener();
    }
    ...
    public Entity targetEntity(Player from, int maxDistance) {
        return ModernTargets.targetEntity(from, maxDistance);
    }

    @Override
    public Block targetBlock(Player from, int maxDistance) {
        return ModernTargets.targetBlock(from, maxDistance);
    }
